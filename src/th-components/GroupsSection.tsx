  ...
  // ФИЛЬТРАЦИЯ ДАННЫХ - по поиску группы, дисциплине и семестру
  const filteredGroups = allGroups.filter(group => {
    ...
    // Фильтр по семестру
    let semesterFilter = true;
    if (activeTab === 'semesters') {
      // Для второго семестра всегда пустой список, исключаем все записи
    ...
    const shouldInclude = groupFilter && disciplineFilter && semesterFilter && courseFilter;

    if (shouldInclude && selectedSemester === 'second') {
      console.log('Группа во втором семестре:', group.numberGroup, {
        groupFilter,
        disciplineFilter,
        semesterFilter,
        courseFilter,
